/* ============================================================================
   ...
   Full CRUD, secure Firebase authentication, stats, seed data, audit logs, backup export.
...
/* ---------------- AUTH ---------------- */
function initAuth() { auth.onAuthStateChanged(async function(user){if(!user){showLogin(false);return;}A.user=user;const ok=await verifyAdmin(user);if(ok)showShell();else{await auth.signOut();showLogin(true);}});const f=el('adminLoginForm');if(f)f.addEventListener('submit',async function(e){e.preventDefault();const email=el('adminEmail')?.value.trim(),pass=el('adminPassword')?.value||'';if(!email||!pass)return toast('Login','Fill all fields','warning');try{await auth.signInWithEmailAndPassword(email,pass);}catch(err){toast('Login',err.message||'Login failed','error');}});const tp=el('toggleAdminPw');if(tp)tp.addEventListener('click',function(){const i=el('adminPassword');if(i)i.type=i.type==='password'?'text':'password';});el('adminLogoutBtn')?.addEventListener('click',()=>auth.signOut());}

async function verifyAdmin(user) { try { const d=await db.collection('admin_users').doc(user.uid).get(); if(!d.exists)return false;const p=d.data()||{};if(p.active===false)return false;A.role=p.role||'admin';A.isAdmin=true;A.permissions=Array.isArray(p.permissions)?p.permissions:[];return true;}catch(e){return false;} }
...
/* ---------------- DASHBOARD ---------------- */
function renderAds(){const host=el('adNetworkControls');if(!host)return;const cfg=A.settings.ads||{};const nets=[['adsterra468','Adsterra 468x60',true],['adsterraNative','Adsterra Native',false],['adsterraPop','Adsterra Popunder',false],['adsterraSocial','Adsterra SocialBar',false],['adsterraSmart','Adsterra Smartlink',false],['monetagTag','Monetag MultiTag',false],['monetagDirect','Monetag Direct Link',false],['hilltop','HilltopAds',false],['popads','PopAds (code later)',false]];host.innerHTML=nets.map(x=>{const on=cfg[x[0]]?.enabled!==undefined?!!cfg[x[0]].enabled:x[2];return '<div class="card"><div class="flex items-center justify-between"><div class="font-bold">'+esc(x[1])+'</div><label class="cbx"><input type="checkbox" data-ad-toggle="'+x[0]+'" '+(on?'checked':'')+'><span class="cbx-box">✓</span></label></div></div>';}).join('');host.querySelectorAll('[data-ad-toggle]').forEach(c=>c.addEventListener('change',async function(){const k=this.dataset.adToggle;const next=Object.assign({},A.settings.ads||{},Object.fromEntries([[k,{enabled:this.checked}]]));await db.collection('settings').doc('global').set({ads:next,updatedAt:serverTimestamp()},{merge:true});A.settings.ads=next;log('ad_toggle',k+':'+this.checked);toast('Ads','Saved','success');}));}
