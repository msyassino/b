...
function initAuthUI() {
  ...
  bind('authModalClose','click',()=>closeModal('authModal'));
  bind('authTabLogin','click',()=>switchAuthPane('login'));
  bind('authTabSignup','click',()=>switchAuthPane('signup'));
  bind('forgotPwLink','click',e=>{e.preventDefault();switchAuthPane('forgot');});
  bind('forgotBackBtn','click',()=>switchAuthPane('login'));
  bind('signupReferral','change',function(){el('referralCodeWrap')?.classList.toggle('hidden',!this.checked);});
  bind('loginForm','submit',async function(e){e.preventDefault();const email=el('loginEmail')?.value.trim(),pw=el('loginPassword')?.value||'';if(!email||!pw)return toast('Login',t('err.fillAll'),'warning');try{await auth.signInWithEmailAndPassword(email,pw);closeModal('authModal');}catch(err){toast('Login',err.message||'Login failed','error');}});
  bind('signupForm','submit',async function(e){e.preventDefault();const un=el('signupUsername')?.value.trim()||'',em=el('signupEmail')?.value.trim()||'',pw=el('signupPassword')?.value||'';if(un.length<3)return toast('Signup',t('err.username'),'warning');if(pw.length<8)return toast('Signup',t('err.password'),'warning');try{if(State.settings.allowNewSignups===false)throw Error('New signups are currently disabled.');const cred=await auth.createUserWithEmailAndPassword(em,pw);const rc=el('signupReferral')?.checked?(el('signupReferralCode')?.value.trim().toUpperCase()||''):'';await createUserProfile(cred.user,un,rc);await cred.user.sendEmailVerification().catch(()=>{});closeModal('authModal');toast('Account','Created successfully. Please verify your email.','success');}catch(err){toast('Signup',err.message||'Signup failed','error');}});
  bind('forgotForm','submit',async function(e){e.preventDefault();const em=el('forgotEmail')?.value.trim();if(!em)return;try{await auth.sendPasswordResetEmail(em);toast('Password reset','Reset email sent.','success');switchAuthPane('login');}catch(err){toast('Password reset',err.message||'Failed','error');}});
  bind('googleLoginBtn','click',async()=>{try{const c=await auth.signInWithPopup(new firebase.auth.GoogleAuthProvider());const d=await colRef('users').doc(c.user.uid).get();if(!d.exists)await createUserProfile(c.user,(c.user.displayName||'user').replace(/\s+/g,'_').slice(0,20),'');closeModal('authModal');}catch(err){toast('Google login',err.message||'Failed','error');}});
  bind('verifyContinueBtn','click',async()=>{if(auth.currentUser){await auth.currentUser.reload();if(auth.currentUser.emailVerified){await colRef('users').doc(auth.currentUser.uid).set({verification:{email:true}},{merge:true});closeModal('verifyModal');}}});
  bind('resendVerifyBtn','click',async()=>{try{await auth.currentUser?.sendEmailVerification();toast('Email','Verification email sent.','success');}catch(e){toast('Email',e.message||'Failed','error');}});
}
...
async function addLedger(uidVal,type,desc,coins,status,ref) {
  status=status||'completed'; const amount=Math.trunc(Number(coins)||0); const reference=ref||('TX-'+uid());
  const userRef=colRef('users').doc(uidVal), ledgerRef=colRef('ledger').doc();
  await db.runTransaction(async tx=>{
    const u=await tx.get(userRef); if(!u.exists)throw Error('User profile not found');
    const p=u.data(); const balance=Number(p.balance||0); if(amount<0 && balance<Math.abs(amount))throw Error('Insufficient coins');
    const patch={balance:balance+amount,lastSeen:serverTimestamp()};
    if(status==='completed'&&amount>0)patch.lifetimeEarned=Number(p.lifetimeEarned||0)+amount;
    if(status==='completed'&&amount<0)patch.lifetimeSpent=Number(p.lifetimeSpent||0)+Math.abs(amount);
    tx.update(userRef,patch);
    tx.set(ledgerRef,{uid:uidVal,type,description:desc,coins:amount,status,reference,ts:Date.now(),createdAt:serverTimestamp()});
  });
}

async function computeWallet(uidVal) {
  const user=await colRef('users').doc(uidVal).get(); const p=user.exists?user.data():{};
  const list=[]; const snap=await colRef('ledger').where('uid','==',uidVal).limit(500).get().catch(()=>null); if(snap)snap.forEach(d=>list.push(Object.assign({id:d.id},d.data())));
  list.sort((a,b)=>(b.ts||0)-(a.ts||0));
  const spent=list.filter(e=>e.status==='completed'&&(e.coins||0)<0).reduce((n,e)=>n+Math.abs(e.coins||0),0);
  const earned=list.filter(e=>e.status==='completed'&&(e.coins||0)>0).reduce((n,e)=>n+(e.coins||0),0);
  const pending=list.filter(e=>e.status==='pending'&&e.coins>0).reduce((n,e)=>n+e.coins,0);
  const locked=list.filter(e=>e.status==='locked'&&e.coins>0).reduce((n,e)=>n+e.coins,0);
  return {coins:Math.max(0,Number(p.balance||0)),pending,locked,earned,spent,withdrawn:Number(p.totalWithdrawn||0),list};
}
...
function navigate(page) {
  ...
  State.currentPage=page;PAGES.forEach(p=>{const s=el('page-'+p);if(s)s.classList.toggle('active',p===page);});$$('[data-nav]').forEach(a=>a.classList.toggle('active',a.getAttribute('data-nav')===page));el('drawer')?.classList.remove('open');window.scrollTo({top:0});
  if(State.settings.maintenance&&page!=='home'&&needsAuth.includes(page)){toast('Maintenance','This section is temporarily unavailable.','warning');return;}
  if(needsAuth.includes(page)&&!State.user){openModal('authModal');return;}renderPage(page);
}
...
function renderEvents() {
  ...
  el('eventsActiveGrid').innerHTML = State.events.filter(function (e) { return e.status === 'active'; }).map(card).join('') || '';
...
/* ---------------- EARNING ACTIONS ---------------- */
async function completeOffer(id) { if(!State.user)return; const o=State.offers.find(x=>x.id===id)||State.games.find(x=>x.id===id); if(!o)return; closeModal('offerModal'); if(!o.link){return toast('Offer','This offer has no provider link configured.','warning');} window.open(o.link,'_blank','noopener'); toast('Offer','Complete it on the provider. Reward is credited after provider verification.','info'); }

async function completeSurvey(id) { if(!State.user)return; const s=State.surveys.find(x=>x.id===id); if(!s)return; closeModal('surveyModal'); if(s.link)window.open(s.link,'_blank','noopener'); toast('Survey','Complete the survey on the provider. Reward is credited after verification.','info'); }

async function redeemReward(id) {
  if(!State.user||State.settings.maintenance)return; const r=State.rewards.find(x=>x.id===id);if(!r)return;const cost=Math.trunc(Number(r.price)||0);if((State.wallet?.coins||0)<cost)return toast('Reward',t('err.insufficient'),'warning');
  try{await db.runTransaction(async tx=>{const u=colRef('users').doc(State.user.uid),rr=colRef('rewards').doc(id),ord=colRef('orders').doc(),led=colRef('ledger').doc();const [ud,rd]=await Promise.all([tx.get(u),tx.get(rr)]);if(!ud.exists||!rd.exists)throw Error('Reward unavailable');const p=ud.data(),rw=rd.data(),stock=Number(rw.stock);if(Number(p.balance||0)<cost)throw Error('Insufficient coins');if(Number.isFinite(stock)&&stock<=0)throw Error('Out of stock');tx.update(u,{balance:Number(p.balance||0)-cost,lifetimeSpent:Number(p.lifetimeSpent||0)+cost});if(Number.isFinite(stock))tx.update(rr,{stock:stock-1});tx.set(led,{uid:State.user.uid,type:'reward',description:t('ledger.rewardRedeem').replace('{n}',rw.title||''),coins:-cost,status:'completed',reference:'REW-'+uid(),orderId:ord.id,ts:Date.now(),createdAt:serverTimestamp()});tx.set(ord,{uid:State.user.uid,type:'reward',item:rw.title,cost,status:'pending',ts:Date.now(),createdAt:serverTimestamp()});});closeModal('confirmModal');await updateBalanceUI();toast('Reward',t('rewards.ordered'),'success');celebrate();}catch(e){toast('Reward',e.message||'Unable to redeem','error');}
}

async function watchAd() { if(!State.user||State.settings.maintenance)return; const used=State.profile?.adsDate===todayKey()?(State.profile?.adsWatchedToday||0):0,cap=Number(State.settings.adDailyCap||15);if(used>=cap)return toast('Ads',t('watch.capReached'),'warning');const url=State.settings.rewardedAdUrl||'https://omg10.com/4/11605558';window.open(url,'_blank','noopener');toast('Ads','Ad started. Reward is credited only after a verified provider callback.','info'); }

async function claimDaily() { if(!State.user||State.settings.maintenance)return;try{const ref='DAY-'+todayKey();await db.runTransaction(async tx=>{const u=colRef('users').doc(State.user.uid),led=colRef('ledger').doc();const d=await tx.get(u);if(!d.exists)throw Error('Profile not found');const p=d.data();if(p.lastClaimDate===todayKey())throw Error(t('daily.claimed'));let streak=Number(p.streak||0);const y=new Date(Date.now()-86400000).toISOString().slice(0,10);if(p.lastClaimDate!==y)streak=0;streak++;const reward=Math.round(DAILY_PLAN[Math.min(6,(streak-1)%7)]*(streak>=7?2:streak>=4?1.5:1));const day=((streak-1)%7)+1,days=Array.isArray(p.claimedDays)?p.claimedDays.slice():[];if(!days.includes(day))days.push(day);tx.update(u,{balance:Number(p.balance||0)+reward,lifetimeEarned:Number(p.lifetimeEarned||0)+reward,streak,bestStreak:Math.max(Number(p.bestStreak||0),streak),lastClaimDate:todayKey(),claimedDays:days,xp:increment(5)});tx.set(led,{uid:State.user.uid,type:'daily',description:t('ledger.dailyClaim').replace('{n}',streak),coins:reward,status:'completed',reference:ref,ts:Date.now(),createdAt:serverTimestamp()});});await updateBalanceUI();renderDaily();showRewardPopup(1,'Daily reward credited.');}catch(e){toast('Daily',e.message,'warning');} }

async function spinWheel() { if(State.settings.maintenance)return;
  if (!State.user) return;
  const pf = State.profile || {};
...
async function scratchCard() { if(State.settings.maintenance)return;
  if (!State.user) return;
  const pf = State.profile || {};
...
async function openMystery() { if(State.settings.maintenance)return;
  if (!State.user) return;
  const pf = State.profile || {};
...
async function openTreasure() { if(State.settings.maintenance)return;
  if (!State.user) return;
  const pf = State.profile || {};
...
async function dailyAdBonus() { if(State.settings.maintenance)return;
  if (!State.user) return;
  const pf = State.profile || {};
...
async function interstitialReward() { if(State.settings.maintenance)return;
  if (!State.user) return;
...
async function confirmTopup() { if(!State.user||State.settings.maintenance)return;const game=document.querySelector('#topupGameList .reward-item.selected'),pkg=document.querySelector('#topupPackageGrid .package.selected'),pid=(el('topupPlayerId')?.value||'').trim();if(!game)return toast('Top Up',t('topup.selectGameFirst'),'warning');if(!pid)return toast('Top Up',t('topup.enterPlayerId'),'warning');if(!pkg)return toast('Top Up','Select a package.','warning');const cost=Math.trunc(Number(pkg.dataset.cost)||0);try{const ref='TOP-'+uid();await db.runTransaction(async tx=>{const u=colRef('users').doc(State.user.uid),ord=colRef('orders').doc(),led=colRef('ledger').doc(),d=await tx.get(u);if(!d.exists)throw Error('Profile not found');const p=d.data();if(Number(p.balance||0)<cost)throw Error('Insufficient coins');tx.update(u,{balance:Number(p.balance||0)-cost,lifetimeSpent:Number(p.lifetimeSpent||0)+cost});tx.set(led,{uid:State.user.uid,type:'topup',description:t('ledger.topup').replace('{g}',game.dataset.game),coins:-cost,status:'completed',reference:ref,ts:Date.now(),createdAt:serverTimestamp()});tx.set(ord,{uid:State.user.uid,type:'topup',item:game.dataset.game,package:pkg.dataset.label,playerId:pid,cost,status:'pending',reference:ref,ts:Date.now(),createdAt:serverTimestamp()});});await updateBalanceUI();toast('Top Up',t('topup.success'),'success');}catch(e){toast('Top Up',e.message,'error');}}

async function requestWithdrawal() { if(!State.user||State.settings.maintenance)return;const amount=Math.trunc(Number(el('wdAmount')?.value||0)),min=Math.trunc(Number(State.settings.minWithdraw||10000));if(amount<min)return toast('Withdraw',t('withdraw.tooSmall').replace('{n}',fmtNum(min)),'warning');if(!State.profile?.verification?.email)return toast('Withdraw','Verify your email before requesting a withdrawal.','warning');if(amount>(State.wallet?.coins||0))return toast('Withdraw',t('err.insufficient'),'warning');try{const ref='WD-'+uid();const existing=await colRef('withdrawals').where('uid','==',State.user.uid).where('status','==','pending').limit(1).get();if(!existing.empty)throw Error(t('withdraw.pendingExists'));await db.runTransaction(async tx=>{const u=colRef('users').doc(State.user.uid),wd=colRef('withdrawals').doc(),led=colRef('ledger').doc(),d=await tx.get(u);if(!d.exists)throw Error('Profile not found');const p=d.data();if(Number(p.balance||0)<amount)throw Error('Insufficient coins');tx.update(u,{balance:Number(p.balance||0)-amount});tx.set(led,{uid:State.user.uid,type:'withdrawal',description:t('ledger.withdrawal')+' · '+State.selectedWdMethod,coins:-amount,status:'completed',reference:ref,ts:Date.now(),createdAt:serverTimestamp()});tx.set(wd,{uid:State.user.uid,amount,method:State.selectedWdMethod,usd:amount/Number(State.settings.coinRate||10000),status:'pending',reference:ref,ts:Date.now(),createdAt:serverTimestamp()});});await updateBalanceUI();toast('Withdraw',t('withdraw.requested'),'success');if(el('wdAmount'))el('wdAmount').value='';updateWdSummary();}catch(e){toast('Withdraw',e.message,'error');}}

async function applyPromo(code) { if(!State.user||State.settings.maintenance)return;const p=State.promos.find(x=>String(x.code).toUpperCase()===String(code||'').trim().toUpperCase()&&x.active!==false);if(!p)return toast('Promo',t('promo.invalid'),'error');try{await db.runTransaction(async tx=>{const u=colRef('users').doc(State.user.uid),led=colRef('ledger').doc(),d=await tx.get(u);if(!d.exists)throw Error('Profile not found');const pf=d.data(),used=Array.isArray(pf.usedPromos)?pf.usedPromos.slice():[];if(used.includes(p.code))throw Error(t('promo.used'));used.push(p.code);const reward=Math.trunc(Number(p.reward)||0);tx.update(u,{balance:Number(pf.balance||0)+reward,lifetimeEarned:Number(pf.lifetimeEarned||0)+reward,usedPromos:used});tx.set(led,{uid:State.user.uid,type:'promo',description:t('ledger.promo').replace('{n}',p.code),coins:reward,status:'completed',reference:'PRM-'+uid(),ts:Date.now(),createdAt:serverTimestamp()});});await updateBalanceUI();showRewardPopup(1,p.code);}catch(e){toast('Promo',e.message,'warning');}}
...
function renderSiteAds(){
  const cfg=State.settings.ads||{};
  const mount=(selector,html)=>document.querySelectorAll(selector).forEach(x=>x.innerHTML=html);
  const script=(src,attrs={})=>{const s=document.createElement('script');s.src=src;s.async=true;Object.entries(attrs).forEach(([k,v])=>s.setAttribute(k,v));return s;};
  const addScript=(host,src,attrs={})=>{host.innerHTML='';host.appendChild(script(src,attrs));};
  const ad468=cfg.adsterra468?.enabled!==false;
  const native=!!cfg.adsterraNative?.enabled;
  const social=!!cfg.adsterraSocial?.enabled;
  const pop=!!cfg.adsterraPop?.enabled;
  const monetag=!!cfg.monetagTag?.enabled;
  const hilltop=!!cfg.hilltop?.enabled;
  const direct=!!cfg.monetagDirect?.enabled;
  document.querySelectorAll('.ad-box-native').forEach(host=>{host.innerHTML='';if(native){const s=script('https://pl30913455.effectivecpmnetwork.com/df7130eb24354334e85ee01b5be086f2/invoke.js',{'data-cfasync':'false'});host.appendChild(s);const d=document.createElement('div');d.id='container-df7130eb24354334e85ee01b5be086f2';host.appendChild(d);}});
  document.querySelectorAll('.ad-box-banner,.ad-box-468,.ad-box').forEach(host=>{if(host.dataset.adRendered)return;if(ad468){host.innerHTML='<span class="ad-label">Advertisement</span><div class="ad468-runtime"></div>';const m=host.querySelector('.ad468-runtime');const c=document.createElement('script');c.textContent="var atOptions={key:'c351ac7c1200d215d77a5b0a74a395fe',format:'iframe',height:60,width:468,params:{}};";m.appendChild(c);m.appendChild(script('https://www.highperformanceformat.com/c351ac7c1200d215d77a5b0a74a395fe/invoke.js'));host.dataset.adRendered='1';}});
  if(social){document.querySelectorAll('.ad-box-social').forEach(host=>addScript(host,'https://pl30913456.effectivecpmnetwork.com/32/aa/62/32aa624c58a1d6ea82421be6e9c8d4b4.js'));}
  if(pop){const s=document.createElement('script');s.src='https://pl30913454.effectivecpmnetwork.com/2e/8e/1d/2e8e1d21821e52cabb4dca2fb31ae1ed.js';s.async=true;document.head.appendChild(s);}
  if(monetag){const s=script('https://quge5.com/88/tag.min.js',{'data-zone':'271240','data-cfasync':'false'});document.head.appendChild(s);}
  if(hilltop){const s=document.createElement('script');s.textContent="(function(krn){var d=document,s=d.createElement('script'),l=d.scripts[d.scripts.length-1];s.settings=krn||{};s.src='//infamous-maximum.com/cKDd9O6.bL2T5/lZS/WQQe9INdzOM/z/MjzNYv0qM/S/0_3FMhzmMwz/NdjIQ/1U';s.async=true;s.referrerPolicy='no-referrer-when-downgrade';l.parentNode.insertBefore(s,l);})({})";document.head.appendChild(s);}
  const top=el('siteAdTop');if(top){top.innerHTML='';if(direct){const a=document.createElement('a');a.href='https://omg10.com/4/11605558';a.target='_blank';a.rel='noopener sponsored';a.className='btn btn-ghost btn-sm';a.textContent='Sponsored offer';top.appendChild(a);}else if(ad468){top.innerHTML='<span class="ad-label">Advertisement</span><div class="ad468-runtime"></div>';const m=top.querySelector('.ad468-runtime');const c=document.createElement('script');c.textContent="var atOptions={key:'c351ac7c1200d215d77a5b0a74a395fe',format:'iframe',height:60,width:468,params:{}};";m.appendChild(c);m.appendChild(script('https://www.highperformanceformat.com/c351ac7c1200d215d77a5b0a74a395fe/invoke.js'));}}
}
...
function watchUser() {
  auth.onAuthStateChanged(async function (user) {
    State.user = user;
    try { const g=await colRef('settings').doc('global').get(); if(g.exists) State.settings=Object.assign(State.settings,g.data()); } catch(e) {}
